                            ...
                            <div class="row justify-content-start">
                                <div class="col-4 border">
                                    <div id="alert_producto">
                                        <?php
                                        if (isset($_GET['aksi']) == "editar") {
                                            $codigo_p = $_GET['codigo_p'];
                                            $sqlproducto = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM cc_productos where id_sucursal = '$id_sucursal' and codigo =" . $codigo_p));
                                            echo '
                                        <div id="' . $codigo_p . '" class="alert alert-secondary alert-dismissible mb-0 align-middle" role="alert"><strong></strong> ' . $sqlproducto['descripcion'] . ' <a href="#" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></a></div>
    ';
                                        }
                                        ?>
                                    </div>
                                ...
                                <div class="col-8 border">
                                    <div id="alert_derivado">
                                        <?php
                                        if (isset($_GET['aksi']) == "editar") {
                                            $sqlderivados = mysqli_query($link, "SELECT * FROM cc_derivados where id_sucursal = '$id_sucursal' and codigo_p = " . $codigo_p);
                                            $renglon = 0;
                                            while ($rowc = mysqli_fetch_assoc($sqlderivados)) {
                                                $sqlproducto = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM cc_productos where id_sucursal = '$id_sucursal' and codigo =" . $rowc['codigo_d']));
                                                echo '
                                        <div id="' . $rowc['codigo_d'] . '" class="alert alert-secondary alert-dismissible mb-0" role="alert" attribute1="' . $rowc['porcentaje'] . '"><strong>' . $rowc['porcentaje'] . '% </strong> ' . $sqlproducto['descripcion'] . ' <a href="#" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></a></div>
        ';
                                            }
                                        }
                                        ?>
                            ...
                            <table id="derivados" class="display" style="width:100%" >
                                ...
                                while ($rowc = mysqli_fetch_assoc($sqlderivados)) {
                                    ...
                                    <tr id="fila' . $renglon . '">
                                        ...
                                        <td align="center">
                                            <a href="?aksi=delete&codigo_p=' . $rowc['codigo_p'] . '" title="Eliminar" onclick="return confirm(\'¿Esta seguro de borrar los derivados de ' . $sqlproducto['descripcion'] . '?\')"><img class="imga" src="../img/icons/trash.svg"></a>
                                            <a href="?aksi=editar&codigo_p=' . $rowc['codigo_p'] . '" title="Editar derivado"><img class="imga" src="../img/icons/pencil-square.svg"></a>
                                        </td>
                                        </tr>
                                        ';
                                }
        ...
        <script>
            ...
            function edita_derivado(codigo_p, codigo_d, porcentaje, movimiento)
            {
                ...
                $.ajax({
                    url: "../functions/edita_derivado.php",
                    data: parametros,
                    dataType: "json",
                    type: "POST",
