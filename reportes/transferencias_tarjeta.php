...
if ($filtro_sucursal > 0) {
    $condicion_sucursal = " AND dv.id_sucursal = $filtro_sucursal ";
}

$condicion_pago = "";
if ($filtro_tipo_pago === 'transferencia') {
    $condicion_pago = " WHERE t.monto_transferencia > 0 ";
} elseif ($filtro_tipo_pago === 'tarjeta') {
    $condicion_pago = " WHERE t.monto_tarjeta > 0 ";
} else {
    $condicion_pago = " WHERE (t.monto_transferencia > 0 OR t.monto_tarjeta > 0) ";
}
...
SELECT * FROM (
    SELECT 
        ...
        COALESCE(tot.total_venta, 0) AS total_venta,
...
$condicion_pago
ORDER BY t.fecha_ingreso DESC, t.hora_ingreso DESC, t.id_venta DESC
";
...
if ($queryReporte) {
    while ($row = mysqli_fetch_assoc($queryReporte)) {
        ...
        $montoVenta = (float) $row['total_venta'];
                ...
                <div>
                    <div class="bg-light p-4 rounded">
                        <div class="col-sm-10 mx-auto">
                            ...
                            <p class="text-center text-muted">Ventas registradas con pago por transferencia bancaria o tarjeta de débito/crédito</p>
                        </div>
                        <br>
                        ...
                        <form class="row g-3 needs-validation mb-4" action="#" method="post">
                            ...
                            <div class="col-md-3">
                                <label for="tipo_pago_filtro" class="form-label font-weight-bold">Filtrar por medio:</label>
                                <select id="tipo_pago_filtro" name="tipo_pago_filtro" class="form-select">
                                    <option value="todos" <?php echo ($filtro_tipo_pago === 'todos') ? 'selected' : ''; ?>>Todos (Transferencia y Tarjeta)</option>
                            ...
                            <div class="col-md-4 mb-2">
                                <div class="card border-primary shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <h3 class="card-text font-weight-bold">$<?php echo number_format($totalTransferencias, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 mb-2">
                                <div class="card border-success shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <h3 class="card-text font-weight-bold">$<?php echo number_format($totalTarjetas, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 mb-2">
                                <div class="card border-dark shadow-sm">
                                    <div class="card-body">
                                        <h5 class="card-title text-dark"><i class="bi bi-cash-stack"></i> Gran Total (Trans + Tarj)</h5>
                                        <h3 class="card-text font-weight-bold">$<?php echo number_format($granTotal, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-sm mx-auto mb-3">
                            <h3 class="text-left">Detalle de Ventas con Transferencia y/o Tarjeta</h3>
                        </div>
                            ...
                            <table id="reporte_tabla" class="display" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    foreach ($ventasData as $v) {
                                        ...
                                        echo '<td class="text-end">$ ' . number_format($v['total_venta'], 2) . '</td>';
                                        echo '</tr>';
                                    }
                                    ?>
                                </tbody>
        ...
        <script>
            $(document).ready(function () {
                $('#reporte_tabla').DataTable({
                    language: {
                        "decimal": "",
                        "emptyTable": "No hay información de transferencias o pagos con tarjeta en el rango seleccionado",
