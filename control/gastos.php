                ...
                <div>
                    <div class="bg-light p-4 rounded ">
                        ...
                        <div class="table-responsive">
                            <table id="gastos" class="display" style="width:100%" >
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Fecha</th>
                                ...
                                <?php
                                $sqlgastos = mysqli_query($link, "SELECT a.id_gasto,a.fecha_ingreso,a.hora_ingreso,"
                                        . "round(a.precio * a.cantidad,2) as importe, a.descripcion "
                                        . "FROM cc_gastos as a "
                                        . "WHERE a.id_sucursal = '$id_sucursal' and a.fecha_ingreso = '$fecha'"
                                        . " order by a.id_gasto DESC");

                                $renglon = 0;
                                while ($rowc = mysqli_fetch_assoc($sqlgastos)) {
                                    $renglon = $renglon + 1;
                                    //$sqlcatalogo = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM cc_catalogos where nombre_clave = 'ROL' and id_clave =" . $rowp['rol']));
                                    //$producto = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM cc_gastos where id_sucursal = '$id_sucursal' and codigo =" . $rowc['codigo']));
                                    ...
                                    <tr id="' . $rowc['id_gasto'] . '">
                                        ...
                                        <td>' . $rowc['importe'] . '</td>
                                        <td align="center">
                                            <a href="#" class=".remove" title="Eliminar" onclick="elimina_gasto(this,' . $rowc['id_gasto'] . ',\'' . $rowc['descripcion'] . '\')"><img class="imga" src="../img/icons/trash.svg"></a>
                                        </td>
                                        </tr>
                                        ';
                                }
                        ...
                        <div class="align-content-center text-center">
                            <a href="#" class="btn btn-primary m-1" role="button" id="addRow">Agrega gasto</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade modal-lg" id="agregaModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form class="needs-validation" action="#" method="post" novalidate id="form_gasto">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary" name="editar">Guardar gasto</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                $('#gastos').dataTable(
                        {
                            ...
                            footerCallback: function () {
                                var api = this.api();
                                $(api.column(3).footer()).html('Total de gasto');
                    ...
                    sUpdateURL: "../functions/actualiza_gasto.php",
                    aoColumns: [
                        null,
                        null,
                        null,
                ...
                $('#addRow').on('click', function () {
                    ...
                    $("#ModalLabelTitle").html("Agregar gasto");
                    $("#importe_e").val("");
                    myModal.show();
                    setTimeout(() => {
                        $("#importe_e").focus();
            ...
            var myForm_gasto = document.getElementById('form_gasto');
            myForm_gasto.addEventListener('submit', function (event) {
                event.preventDefault(); // Evitar que el formulario se envíe automáticamente
                // Validar el formulario
                if (myForm_gasto.checkValidity() === false) {
                    // Si hay errores, no hacer nada
                    event.stopPropagation();
                } else {
                    guarda_gasto(); // Llama a la función JavaScript para agregar a la tabla
                    $('#agregaModal').modal('hide');
                }
                // Agregar la clase "was-validated" para mostrar los errores
                myForm_gasto.classList.add('was-validated');
            });
            function guarda_gasto()
            {
                ...
                $.ajax({
                    url: "../functions/edita_gastos.php",
                    data: parametros,
                    dataType: "json",
                    type: "POST",
                    success: function (response) {
                        var tabla = $("#gastos").DataTable();
                        tabla.row.add([response[0].id_gasto, response[0].fecha, response[0].hora, response[0].descripcion, response[0].importe, '<a href="#" class=".remove" onclick="elimina_gasto(this,' + response[0].id_gasto + ',\'' + response[0].descripcion + '\')" title="Eliminar gasto" class=".remove"><img src="../img/icons/trash.svg"></a>']).draw(false);
                    },
            ...
            function elimina_gasto(objeto, id_gasto, descripcion)
            {
                if (confirm("¿Desea eliminar el gasto " + descripcion + "?"))
                {
                    movimiento = 2;
                    //id_consecutivo = $("#id_consecutivo").val();
                    var parametros = {"movimiento": movimiento, "id_gasto": id_gasto};
                    $.ajax({
                        url: "../functions/edita_gastos.php",
                        data: parametros,
                        dataType: "json",
