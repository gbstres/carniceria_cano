...
(function ($) {
    ...
    $.fn.makeEditable = function (options) {
        ...
        function _fnApplyEditable(aoNodes) {
            ...
            var oDefaultEditableSettings = {
                ...
                "callback": function (sValue, settings) {
                    ...
                    properties.fnOnEdited(status, sOldValue, sNewCellDisplayValue, aPos[0], aPos[1], aPos[2], idTr);
                ...
                "onsubmit": function (settings, original) {
                    var input = $("input,select,text,textarea,number", this);
                    sOldValue = original.revert;
                    sNewCellValue = $("input,select,text,textarea,number", $(this)).val();
                    if (input.length == 1) {
                ...
                "submitdata": function (value, settings) {
                    ...
                    var sColumnName = settings.sColumnName;
            ...
            if (properties.aoColumns != null) {
                for (var i = 0; i < properties.aoColumns.length; i++) {
                    if (properties.aoColumns[i] != null) {
						var filaRemovida = aoNodes.filter(function(elemento) {
							return $(elemento).hasClass('read_only');
						});
						filaRemovida.forEach(function(elemento) {
						var indice = aoNodes.indexOf(elemento);
						if (indice !== -1) {
							aoNodes.splice(indice, 1);
							}
						});
        ...
        */
        ...
        function fnOnEdited(result, sOldValue, sNewValue, iRowIndex, iColumnIndex, iRealColumnIndex, idTr) {
